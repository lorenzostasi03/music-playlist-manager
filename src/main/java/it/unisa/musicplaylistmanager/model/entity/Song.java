...
public class Song {
	 ...
	 *             se uno dei parametri non è valido.
	 */
	...
	/**
	 * Crea una traccia musicale a partire da dati già persistenti. Questo
	 * costruttore viene utilizzato per ricostruire un oggetto Song già esistente
	 * nel sistema.
	 *
	 * @param id
	 *            identificatore univoco del brano.
	 * @param title
	 *            titolo della traccia.
	 * @param author
	 *            autore della traccia.
	 ...
	 */
	public Song(UUID id, String title, String author, Genre genre, int year, int duration, String filePath,
			int playCount) {
		this.id = id;
		this.title = title.trim();
		this.author = author.trim();
		...
		// carica una lista vuota di tag (da modificare quando si aggiungeranno i tag
		// nel DB)
		this.tags = EnumSet.noneOf(Tag.class);
	}
	...
	/**
	 * Aggiunge un tag alla traccia. Se il tag è già presente, l'operazione non ha
	 * alcun effetto.
	 *
	 * @param tag
	 *            tag da aggiungere;
	...
	/**
	 * Restituisce una rappresentazione testuale della traccia con i metadati
	 * principali.
	 *
	...
	public String toString() {
		return String.format("Song{id='%s', title='%s', author='%s', genre=%s, year=%d}", id, title, author, genre,
				year);
	}
	...
	/**
	 * Valida che l'anno sia un valore positivo e non superiore all'anno corrente +
	 * 1
	 *
	 *
	 * @param year
	...
	private void validateYear(int year) {
		...
		if (year <= 0 || year > annoCorrente + 1) {
			throw new IllegalArgumentException("L'anno di pubblicazione non è valido: " + year);
		}
	}
	...
	private void validateDuration(int duration) {
		if (duration < 0) {
			throw new IllegalArgumentException("La durata non può essere negativa: " + duration);
		}
	}
}
