...
public class Playlist {
     ...
     * La playlist viene creata vuota, senza tracce.
     *
    ...
    public void addSong(Song song) {
        ...
        if (contains(song)) {
            throw new IllegalArgumentException(
                "La traccia '" + song.getTitle() + "' è già presente nella playlist.");
        }
        songs.add(song);
    }
    ...
    public void removeSong(Song song) {
        ...
        if (!contains(song)) {
            throw new IllegalArgumentException(
                "La traccia '" + song.getTitle() + "' non è presente nella playlist.");
        }
        songs.remove(song);
    }
     ...
     *
     * @param song        traccia da spostare che deve essere presente nella playlist
    ...
    public void moveSong(Song song, int newPosition) {
        ...
        if (newPosition < 0 || newPosition >= songs.size()) {
            throw new IllegalArgumentException(
                "Posizione non valida: " + newPosition +
                    ". Range consentito: [0, " + (songs.size() - 1) + "].");
        }
        songs.remove(song);
     ...
     *
     * @param index indice della traccia;
     * @return traccia all'indice indicato
     */
    public Song getSongAt(int index) {
        if (index < 0 || index >= songs.size()) {
            throw new IndexOutOfBoundsException(
                "Indice non valido: " + index);
        }
    ...
    public String toString() {
        return String.format("Playlist{name='%s', tracce=%d, riproduzioni=%d}",
            name, songs.size(), playCount);
    }
