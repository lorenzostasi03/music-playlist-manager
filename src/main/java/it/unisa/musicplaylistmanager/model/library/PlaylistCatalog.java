...
public class PlaylistCatalog {
	...
	/**
	 * Verifica che il nuovo nome di una playlist non sia già in uso da un'altra
	 * playlist
	 *
	 * @param currentName
	 *            nome attuale della playlist da rinominare
	 * @param newName
	 *            nuovo nome proposto
	...
	public boolean isNameTakenByOther(String currentName, String newName) {
		return playlists.values().stream().filter(p -> !p.getName().equalsIgnoreCase(currentName))
	...
	public boolean existsByName(String name) {
		if (name == null)
			return false;
		return playlists.values().stream().anyMatch(p -> p.getName().equalsIgnoreCase(name.trim()));
	}
