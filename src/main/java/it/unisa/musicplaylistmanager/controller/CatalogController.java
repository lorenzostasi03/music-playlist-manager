 ...
 * base a vari criteri, e accedere alle funzioni di aggiunta, modifica ed
 * eliminazione.
 */
public class CatalogController {
	...
	private void initialize() {
		...
		genreFilter.getItems().setAll("Tutti", "Pop", "Rock", "Hip-Hop", "Jazz", "Classical", "Electronic", "R&B",
				"Country", "Metal", "Indie", "Folk", "Reggae", "Blues", "Altro");
	...
	private void refreshCatalog() {
		...
		List<Song> songs = App.getMusicLibrary().getAllSongs().stream().toList();
	...
	private void refreshFilterValues() {
		...
		List<String> authors = App.getMusicLibrary().getAllSongs().stream().map(Song::getAuthor).distinct()
				.sorted(String.CASE_INSENSITIVE_ORDER).collect(Collectors.toList());
		authors.add(0, "Tutti");

		List<String> years = App.getMusicLibrary().getAllSongs().stream().map(song -> String.valueOf(song.getYear()))
